...
DRPC routers compose procedures into a namespaced tree. They provide hierarchical access to procedures (`api.users.list()`) and enable modular API design. Routers can be nested, merged, and have middleware applied at the router level.
...
A router is a named collection of procedures and sub-routers. It creates a hierarchical namespace:
...
const merged = d.router(
  d.router({ users: { list: ..., byId: ... } }),
  d.router({ posts: { list: ..., byId: ... } }),
);
...
const globalRouter = d.router({
  users: { list: ..., byId: ..., create: ... },
  posts: { list: ..., byId: ..., create: ... },
}).use(loggingMiddleware);
```
...
The type system ensures routers contain valid procedures:
...
Routers can carry metadata for documentation or adapter configuration:
...
Routers are standalone — they don't require a full DRPC instance:
...
export const usersRouter = d.router({
  list: d.query({
    ...
    handler: async (ctx) => ok(ctx.db.listUsers()),
  }),
  byId: d.query({
    meta: { authRequired: true },
    handler: async (ctx, args) => ok(ctx.db.findUser(args.id)),
...
### Sharing Routers Across Projects

Since routers are just data structures, they can be shared:

```typescript
// In a shared library @myorg/shared-routers
export const baseRouter = d.router({
  health: d.query({ handler: async () => ok({ status: 'ok' }) }),
  version: d.query({ handler: async () => ok({ version: '1.0.0' }) }),
});

// In your application
const router = d.router({
  ...baseRouter,  // Spread the shared router
  users: { ... }, // Add application-specific
});
```
...
Creates a router from one or more route objects. Routes can be nested.
...
class Router<TCtx> {
  // Get nested router
  get<TRoutes extends DecoratedRouter<TCtx>>(routes: TRoutes): Router<TCtx>;
...
```
users: { list: proc }     → path: "users.list"
v1: { users: { list: proc } } → path: "v1.users.list"
```
