...
To trigger cache invalidation after a mutation, you return an `ok()` result with an `invalidate` option. This option accepts an array of key paths that should be cleared from the cache. The keys follow the same structure you use when defining queries with `t.query`.

When you return `ok(value, { invalidate: [...] })`, the framework automatically clears any cached query entries matching those keys. On the client side, React components watching those keys will automatically refetch to keep the UI in sync with the server state.
