...
Use `context` when you want the exact object you provide to be used as context. This works best for already-initialized dependencies like singletons or connection pools:
...
Use `createContext` when you want the factory to run only when `createAPI()` is called, not at module load time. This is best for delaying expensive initialization:
...
This section explains when context is created:
