...
Queries fetch data from your API without modifying it. They are exposed as HTTP GET endpoints and support automatic caching based on the keys you return. Use queries for any operation that reads data.
...
`t.query()` is a higher-order function. It takes a definition object with `args` and `handler`, and returns a procedure you can call or register in a router:
...
`t.query()` returns a procedure - a function with metadata attached. You can:
