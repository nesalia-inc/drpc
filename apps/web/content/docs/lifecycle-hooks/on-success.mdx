...
const riskyOperation = t.query({
  args: z.object({ id: z.number() }),
  handler: async (ctx, args) => {
    const result = await ctx.db.riskyOperation(args.id)
    if (!result.ok) {
  ...
  // This never runs when result is an error
  // If you find yourself checking result.ok here, use onError instead
})
```
