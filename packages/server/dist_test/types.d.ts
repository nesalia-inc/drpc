...
export type AfterInvokeHook<Ctx, Args, Output> = (ctx: Ctx, args: Args, result: Result<Output>) => void | Promise<void>;
...
export type Router<Ctx = unknown, Routes extends Record<string, unknown> = Record<string, unknown>> = {
    [K in keyof Routes & string]: Routes[K] extends Procedure<Ctx, infer _Args, infer _Output> ? Routes[K] : Routes[K] extends Record<string, unknown> ? Router<Ctx, Routes[K]> : never;
};
export type Procedure<Ctx, Args, Output> = Query<Ctx, Args, Output> | Mutation<Ctx, Args, Output> | InternalQuery<Ctx, Args, Output> | InternalMutation<Ctx, Args, Output>;
